...
function UILib.new(name, size, watermarkActivity)
    ...
    -- theme
    self._color_accent = Color3.fromRGB(255, 0, 255)
    self._color_text = Color3.fromRGB(255, 255, 255)
    self._color_crust = Color3.fromRGB(10, 10, 10)
    self._color_border = Color3.fromRGB(10, 10, 10)
    self._color_surface = Color3.fromRGB(10, 10, 10)
    self._color_overlay = Color3.fromRGB(10, 10, 10)
...
function UILib:CreateSettingsTab(customName)
    ...
    local presetThemes = {'Freemont', 'Nord', 'Dracula', 'Catppuccin'}
    self:Choice(menuTab, menuTheme, 'Preset theme', {presetThemes[1]}, function (values)
        ...
        if theme == presetThemes[1] then
            colorAccent.value = {255, 0, 255}
            colorBase.value = {10, 10, 10}
            colorInnerStroke.value = {10, 10, 10}
            colorOuterStroke.value = {10, 10, 10}
    ...
    self:Colorpicker(menuTab, menuTheme, 'Accent', {255, 0, 255}, function (newColor)
        self._color_accent = newColor
    end)
    self:Colorpicker(menuTab, menuTheme, 'Base', {10, 10, 10}, function (newColor)
        self._color_surface = newColor
    end)
    self:Colorpicker(menuTab, menuTheme, 'Inner stroke', {10, 10, 10}, function (newColor)
        self._color_border = newColor
    end)
    self:Colorpicker(menuTab, menuTheme, 'Outer stroke', {10, 10, 10}, function (newColor)
        self._color_overlay = newColor
    end)
    self:Colorpicker(menuTab, menuTheme, 'Crust', {10, 10, 10}, function (newColor)
...
local myGui = UILib.new("Fiji's Prison Life", Vector2.new(350, 500), {getTime})


-- defines for player and char
...
-- create tabs
local miscTab = myGui:Tab('Misc')
local WepTab = myGui:Tab('Weapons')
local tpTab = myGui:Tab('Teleports')
myGui:CreateSettingsTab('GUI Config')

-- create sections
local tpSection = myGui:Section(tpTab, 'TP Stuff')
local utilSection = myGui:Section(miscTab, 'Utilities')
local wepSection = myGui:Section(WepTab, 'Gun Grab')

-- util elements


-- tp elements
myGui:Button(tpTab, tpSection, 'CBase', function()

    local X = -976.6282958984375
    local Y = 108.3236846923828
    local Z = 2049.762451171875
    ...
    if not success then
        humanoidRootPart.Position = humanoidRootPart.Position + Vector3.new(0, 5, 0)
        wait(0.1)
        humanoidRootPart.Position = Vector3.new(X, Y, Z)
    end
    print("TP Attempted, should have worked")
end)


myGui:Button(tpTab, tpSection, 'GArmory', function()


    local X = 801.6097412109375
    local Y = 97.98832702636719
    local Z = 2298.1298828125
...
myGui:Button(tpTab, tpSection, 'GTower', function()


    local X = 820.9572143554688
    local Y = 125.83999633789062
    local Z = 2588.590087890625
...
-- misc elements
myGui:Button(miscTab, utilSection, 'Position Finder', function()
    print("X = " .. humanoidRootPart.Position.X .. " Y = " .. humanoidRootPart.Position.Y .. " Z = " .. humanoidRootPart.Position.Z)
end)

myGui:Button(miscTab, utilSection, 'Become Criminal', function()
    ...
    print("Should be a criminal now")
end)

-- weapon elements

_isGrabbingAK47 = false
myGui:Button(WepTab, wepSection, 'Grab AK-47', function()
    if _isGrabbingAK47 then
        print("Already grabbing AK-47")
        return
    end
    
    _isGrabbingAK47 = true
    ...
    local originalX = humanoidRootPart.Position.X
    local originalY = humanoidRootPart.Position.Y
    local originalZ = humanoidRootPart.Position.Z
    
    print("Original position: " .. originalX .. ", " .. originalY .. ", " .. originalZ)
    
    local weaponX = -922.399658203125
    local weaponY = 94.36839294433594
    local weaponZ = 2052.09130859375

    local success, errorMsg = pcall(function()
        humanoidRootPart.Position = Vector3.new(weaponX, weaponY, weaponZ)
    end)
    
    if success then
        print("Teleported to AK-47 position")
    else
        print("Failed to teleport to weapon: " .. tostring(errorMsg))
        _isGrabbingAK47 = false
        return
    end
    ...
    local returnSuccess, returnError = pcall(function()
        humanoidRootPart.Position = Vector3.new(originalX, originalY, originalZ)
    end)
    
    if returnSuccess then
        print("Returned to original position: " .. originalX .. ", " .. originalY .. ", " .. originalZ)
    else
        print("Failed to return: " .. tostring(returnError))
    end
...
_isGrabbingRemington = false
myGui:Button(WepTab, wepSection, 'Grab Remington', function()
    if _isGrabbingRemington then
        print("Already grabbing Remington")
        return
    end
    
    _isGrabbingRemington = true
    ...
    if success then
        print("Teleported to Remington pos")
    else
        print("Failed to teleport to weapon: " .. tostring(errorMsg))
        _isGrabbingRemington = false
        return
...
_isGrabbingM9 = false
myGui:Button(WepTab, wepSection, 'Grab M9', function()
    if _isGrabbingM9 then
        print("Already grabbing M9")
        return
    end
    ...
    local weaponX = 813.7658081054688
    local weaponY = 100.94000244140625
    local weaponZ = 2217.18603515625
...
print("GUI Loaded!! Enjoy Fiji's Prison Life GUI. I spent some time on it :) pls give me script dev for this vault")
